...
<xsl:stylesheet 
     ...
     <xsl:template match="TEI:body//TEI:div/(TEI:p|TEI:seg|TEI:l|TEI:head)/text()">
          <xsl:analyze-string select="substring(.,1,3)" regex="»{{2}}\w"><xsl:matching-substring><xsl:value-of select="upper-case(.)"/></xsl:matching-substring><xsl:non-matching-substring><xsl:value-of select="."/></xsl:non-matching-substring></xsl:analyze-string>
          <xsl:copy-of select="substring(.,4,10000000000000000)"/>
     </xsl:template>
