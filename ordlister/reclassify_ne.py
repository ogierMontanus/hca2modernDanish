...
Usage:
  python reclassify_ne.py [--tsv wordlist_ne_scored.tsv]
                          [--ddo ../../ordbøger/ddo-dsl/ddo_DDO.dic]
                          [--ne-list ../../normalisering/rules/named_entities.txt]
                          [--rules ../../normalisering/rules/rules.tsv]
...
_ROOT = _HERE.parent
sys.path.insert(0, str(_ROOT / "normalisering" / "tools"))
...
def main() -> None:
    ...
    ap.add_argument("--tsv", type=Path,
                    default=_HERE / "wordlist_ne_scored.tsv")
    ap.add_argument("--ddo", type=Path,
                    default=_ROOT / "ordbøger" / "ddo-dsl" / "ddo_DDO.dic")
    ap.add_argument("--ne-list", type=Path,
                    default=_ROOT / "normalisering" / "rules" / "named_entities.txt")
    ap.add_argument("--rules", type=Path,
                    default=_ROOT / "normalisering" / "rules" / "rules.tsv")
